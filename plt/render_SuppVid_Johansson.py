...
# # #  P A R A M  # # #
bShowOnScreen = True        # Show the rending process on screen? (slower)
SAVE = True                 # Save to video disk?

# # #  Johansson
dsl = "2021-08-17-18-30-38-392007_020_3_dots_Johansson_1973"  
x0 = np.array([(0.,0.,0.), (-0.10,0.,0.60)])        # Assumed starting locations of shape (2,K)
tStart, tEnd = 0., 20.       # start and end time of the simulation (before speed factor)
...
bHideLinearDot = False       # Just for demo
...
sourcecolors = np.vstack([si.colors.get_color(si.colors.CMAP['glo'], 0 ,1), si.colors.get_colors(si.colors.CMAP['ind'], 3)])
dotcolors = si.colors.get_colors(si.colors.CMAP['velo'], 3)
...
wc, hc = 0.10 * M / K * fig.get_figheight()/fig.get_figwidth() , 0.10
...
if SAVE:
    ani.save(filename="vid/video_S1_Johansson.mp4", writer=None, fps=fps, dpi=150, codec=None, bitrate=2048, extra_args=['-pix_fmt', 'yuv420p'])  # -pix_fmt yuv420p should improve Mac compatibility
